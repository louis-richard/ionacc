...
tints_brst = [
    ["2017-07-23T16:54:14.000000", "2017-07-23T16:59:25.745247"],
...
def _delta_k(inp_sour, err_sour, inp_peak, err_peak):
    ...
    with warnings.catch_warnings():
        ...
        f_avg = interpolate.interp1d(
            np.log10(vdf_peak_avg), ene_peak, fill_value="extrapolate"
        )
        f_min = interpolate.interp1d(
            np.log10(vdf_peak_min), ene_peak, fill_value="extrapolate"
        )
        f_max = interpolate.interp1d(
            np.log10(vdf_peak_max), ene_peak, fill_value="extrapolate"
        )
    ...
    indices_energ = np.where(
        np.logical_and(~np.isnan(delta_ene_max), ~np.isnan(delta_ene_min))
    )[0]
    delta_ene_err = [
    ...
    ene_peak_ = xr.DataArray(
        ene_peak_avg_, coords=[inp_sour.energy.data], dims=["energy"]
    )
    delta_ene = xr.DataArray(
        delta_ene_avg,
        coords=[delta_ene_ene],
        dims=["energy"],
...
def main(args):
    ...
    dpf_allt_p, cts_allt_p = load_eis_allt_mmsx("proton", tints_brst[args.t_id], cfg)

    cts_omni_mms, dpf_skymap_mms, cts_skymap_mms = [], [], []

    for dpf, cts in zip(dpf_allt_p, cts_allt_p):
        cts_omni_mms.append(
            mms.eis_omni(mms.eis_spin_avg(cts, method="sum"), method="sum")
        )
        dpf_skymap_mms.append(mms.eis_skymap(mms.eis_ang_ang(dpf)))
        cts_skymap_mms.append(mms.eis_skymap(mms.eis_ang_ang(cts), to_psd=False))
    ...
    vdf_eis_i.phi.data += 180.0
    vdf_eis_i.theta.data += 90.0
    ...
    cts_eis_i.phi.data += 180.0
    cts_eis_i.theta.data += 90.0

    cts_eis_i_sfrm = cts_eis_i
    cts_eis_i_ifrm = mms.vdf_frame_transformation(cts_eis_i_sfrm, 1e3 * v_gse_i)
    ...
    delta_en_sfrm, e_peak_sfrm = _delta_k(
        vdf_eis_omni_sour_sfrm,
        erd_eis_omni_sour_sfrm,
        vdf_eis_omni_peak_sfrm,
        erd_eis_omni_peak_sfrm,
    )

    delta_en_ifrm, e_peak_ifrm = _delta_k(
        vdf_eis_omni_sour_ifrm,
    ...
    if args.t_id == 0:
        ...
        plot_line(
            axs00[2],
            -1e-3 * pyrf.cross(v_gsm_i, b_gsm)[:, 1],
            label="$-(V_i\\times B)_y$",
        )
        ...
        gsp = fig.add_gridspec(
            20, 2, top=0.95, bottom=0.06, left=0.08, right=0.92, hspace=0.0
        )

        gsp0 = gsp[:9, :].subgridspec(3, 1, hspace=0)
        gsp1 = gsp[11:, :].subgridspec(1, 3, hspace=0.3, wspace=0.3)
    ...
    span_tint(
        axs00, tint_post, facecolor="tab:olive", linestyle="-", edgecolor="k", alpha=0.2
    )
    axs00[0].text(
    ...
    axs10[0].errorbar(
        vdf_eis_omni_peak_sfrm.energy.data / 1e3,
        vdf_eis_omni_peak_sfrm.data,
        erd_eis_omni_peak_sfrm,
        color="tab:olive",
        linestyle="-",
        marker="s",
    ...
    axs10[1].errorbar(
        vdf_eis_omni_peak_ifrm.energy.data / 1e3,
        vdf_eis_omni_peak_ifrm.data,
        erd_eis_omni_peak_ifrm,
        color="tab:olive",
        linestyle="-",
        marker="s",
    ...
    axs10[2].errorbar(
        delta_en_sfrm.energy.data,
        delta_en_sfrm.data,
        delta_en_sfrm.attrs["err"],
        marker="s",
        capsize=4,
        color="tab:cyan",
        label="Spacecraft frame",
    )
    axs10[2].errorbar(
        delta_en_ifrm.energy.data,
        delta_en_ifrm.data,
        delta_en_ifrm.attrs["err"],
        marker="s",
        capsize=4,
        color="tab:purple",
        label="Proton bulk frame",
    )
    ...
    # Print characteristic quantities (<B>, <V_i>, \\tau, L_x) to console
    print(
        f"Event {'I' * args.t_id + 1} \n"
        f"\t<B> = {b_avg:3.2f} nT ; "
        f"\t<V_i> = {v_avg:3.2f} ± {v_std:3.2f} km s^{{-1}} ; "
        f"\t\\tau = {delta_t:3.2f} s ; "
        f"\tL_x = {x_scale.nominal_value:3.2f} ± {x_scale.std_dev:3.2f} R_E"
    )
    ...
    make_labels(axs00, [0.01, 0.85], 0)

    if args.t_id == 0:
        make_labels(axs10, [0.025, 0.94], 4)
    else:
        make_labels(axs10, [0.025, 0.94], 3)
