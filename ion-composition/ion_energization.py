...
from pyrfu.mms import vdf_omni, eis_skymap, eis_skymap_combine_sc, eis_ang_ang
...
def calc_ratio(dpf_sour, dpf_peak, cts_sour, cts_peak):
    ...
    flux_sour = unumpy.uarray(dpf_sour.data, dpf_sour.data / np.sqrt(cts_sour.data))
    flux_peak = unumpy.uarray(dpf_peak.data, dpf_peak.data / np.sqrt(cts_peak.data))
...
def main(args):
    ...
    dpf_eis_p.phi.data += 180.0
    dpf_eis_a.phi.data += 180.0
    dpf_eis_p.theta.data += 90.0
    dpf_eis_a.theta.data += 90.0
    ...
    dpf_hpca_peak_a.data[dpf_hpca_peak_a.data == 0.0] = np.nan
    ...
    dpf_hpca_sour_a.data[dpf_hpca_sour_a.data == 0.0] = np.nan

    # Compute post/pre-enhancement flux ratio.
    dpf_ratio_p, err_ratio_p = calc_ratio(
        dpf_eis_sour_p, dpf_eis_peak_p, cts_eis_sour_p, cts_eis_peak_p
    )
    dpf_ratio_a, err_ratio_a = calc_ratio(
        dpf_eis_sour_a, dpf_eis_peak_a, cts_eis_sour_a, cts_eis_peak_a
    )
    ...
    idx_p = np.where(
        np.logical_and(
            dpf_eis_sour_p.energy.data > 50.0, ~np.isnan(dpf_eis_sour_p.data)
        )
    )[0]
    slope_sour_p = np.polyfit(
        np.log10(dpf_eis_sour_p.energy.data[idx_p]),
        np.log10(dpf_eis_sour_p.data[idx_p]),
        1,
    )[0]
    idx_p = np.where(
        np.logical_and(
            dpf_eis_peak_p.energy.data > 50.0, ~np.isnan(dpf_eis_peak_p.data)
        )
    )[0]
    slope_peak_p = np.polyfit(
        np.log10(dpf_eis_peak_p.energy.data[idx_p]),
        np.log10(dpf_eis_peak_p.data[idx_p]),
        1,
    )[0]

    idx_a = np.where(
        np.logical_and(
            dpf_eis_sour_a.energy.data > 2 * 50.0, ~np.isnan(dpf_eis_sour_a.data)
        )
    )[0]
    slope_sour_a = np.polyfit(
        np.log10(dpf_eis_sour_a.energy.data[idx_a]),
        np.log10(dpf_eis_sour_a.data[idx_a]),
        1,
    )[0]
    idx_a = np.where(
        np.logical_and(
            dpf_eis_peak_a.energy.data > 2 * 50.0, ~np.isnan(dpf_eis_peak_a.data)
        )
    )[0]
    slope_peak_a = np.polyfit(
        np.log10(dpf_eis_peak_a.energy.data[idx_a]),
        np.log10(dpf_eis_peak_a.data[idx_a]),
        1,
    )[0]
    ...
    gsp = fig.add_gridspec(
        15, 2, top=0.95, bottom=0.05, left=0.12, right=0.88, hspace=0.1
    )

    gsp0 = gsp[:6, :].subgridspec(3, 1, hspace=0)
    gsp1 = gsp[7:, :].subgridspec(2, 2, hspace=0.3, wspace=0.3)
    ...
    axs00[0].legend(
        ["$B_x$", "$B_y$", "$B_z$", "|B|"], frameon=True, ncol=4, loc="upper right"
    )
    axs00[0].set_ylabel("$B$" + "\n" + "[nT]")
    axs00[0].set_ylim([-8, 15])

    axs00[1], caxs001 = plot_spectr(
        axs00[1], dpf_eis_omni_p, yscale="log", cscale="log"
    )
    axs00[1].set_ylim([15, 300])
    axs00[1].set_ylabel("$K_{H^{+}}$" + "\n" + "[keV]")
    caxs001.set_ylabel("Diff. Flux H$^{+}$" + "\n" + "[(cm$^{2}$ s sr keV)$^{-1}$]")

    axs00[2], caxs002 = plot_spectr(
        axs00[2], dpf_eis_omni_a, yscale="log", cscale="log"
    )
    ...
    # Proton
    # Pre enhancement
    err_hp_s = axs10[0].errorbar(
        dpf_eis_sour_p.energy.data,
        dpf_eis_sour_p.data,
        err_eis_sour_p,
        color="tab:orange",
        linestyle="-",
    ...
    # Post enhancement
    err_hp_e = axs10[0].errorbar(
        dpf_eis_peak_p.energy.data,
        dpf_eis_peak_p.data,
        err_eis_peak_p,
        color="tab:green",
        linestyle="-",
    ...
    # Helium
    # Pre enhancement
    err_he_s = axs10[0].errorbar(
        dpf_eis_sour_a.energy.data,
        dpf_eis_sour_a.data,
        err_eis_sour_a,
        capsize=4,
        color="tab:red",
    ...
    # Post enhancement
    err_he_e = axs10[0].errorbar(
        dpf_eis_peak_a.energy.data,
        dpf_eis_peak_a.data,
        err_eis_peak_a,
        color="k",
        linestyle="-",
    ...
    axs10[0].plot(
        dpf_hpca_sour_a.energy.data / 1e3,
        dpf_hpca_sour_a.data,
        linestyle="-",
        color="tab:red",
    )
    axs10[0].plot(
        dpf_hpca_peak_a.energy.data / 1e3,
        dpf_hpca_peak_a.data,
        linestyle="-",
        color="k",
    )
    ...
    l00 = axs10[0].legend(
        [(err_hp_s, err_he_s), (err_hp_e, err_he_e)],
        ["Source (s.)", "Energized (e.)"],
        loc="lower left",
        title="Population",
        frameon=True,
        handler_map={
    ...
    axs10[0].legend(
        [(err_hp_s, err_hp_e), (err_he_s, err_he_e)],
        ["H$^{+}$", "He$^{2+}$"],
        loc="upper right",
        title="Species",
        frameon=True,
    ...
    # Proton
    # Pre enhancement
    err_hp_s = axs10[1].errorbar(
        dpf_eis_sour_p.energy.data / 1,
        dpf_eis_sour_p.data,
        err_eis_sour_p,
        color="tab:orange",
        linestyle="-",
    ...
    # Post enhancement
    err_hp_e = axs10[1].errorbar(
        dpf_eis_peak_p.energy.data / 1,
        dpf_eis_peak_p.data,
        err_eis_peak_p,
        color="tab:green",
        linestyle="-",
    ...
    # Helium
    # Pre enhancement
    err_he_s = axs10[1].errorbar(
        dpf_eis_sour_a.energy.data / 2,
        dpf_eis_sour_a.data,
        err_eis_sour_a,
        color="tab:red",
        linestyle="-",
    ...
    # Post enhancement
    err_he_e = axs10[1].errorbar(
        dpf_eis_peak_a.energy.data / 2,
        dpf_eis_peak_a.data,
        err_eis_peak_a,
        color="k",
        linestyle="-",
    ...
    axs10[1].plot(
        dpf_hpca_sour_a.energy.data / 2e3,
        dpf_hpca_sour_a.data,
        linestyle="-",
        color="tab:red",
    )
    axs10[1].plot(
        dpf_hpca_peak_a.energy.data / 2e3,
        dpf_hpca_peak_a.data,
        linestyle="-",
        color="k",
    )
    ...
    make_labels(axs00, [0.01, 0.87], pad=0)
    make_labels(axs10, [0.025, 0.92], pad=3)
    make_labels(axs11, [0.025, 0.92], pad=5)

    span_tint(
        [axs00[0]], tint_pre_plot, facecolor="tab:orange", linestyle="none", alpha=0.2
    )
    span_tint([axs00[0]], tint_post, facecolor="tab:olive", linestyle="none", alpha=0.2)
    span_tint(axs00, tint_pre_plot, facecolor="none", linestyle="--", edgecolor="k")
    span_tint(axs00, tint_post, facecolor="none", linestyle="-", edgecolor="k")
    axs00[0].text(
        (mdates.datestr2num(tint_pre_plot[1]) + mdates.datestr2num(tint_pre_plot[0]))
        / 2,
        13,
        "s.",
        ha="center",
    ...
    axs00[0].text(
        (mdates.datestr2num(tint_post[1]) + mdates.datestr2num(tint_post[0])) / 2,
        13,
        "e.",
        ha="center",
        color="k",
    ...
    fig.suptitle(f"Event {'I' * (args.t_id + 1)} " f"({tint[0][:-7]} - {tint[1][:-7]})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--t-id", help="Time interval index", required=True, type=int)
