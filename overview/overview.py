...
def main(args):
    ...
    # %% Compute plasma beta
    p_tot = 1e-9 * p_p + 1e-9 * resample(p_e, p_p)     # Plasma pressure
    ...
    for i, c_fpi, c_hpca in zip(range(3), colors_i, colors_p):
        plot_line(axs11[2], v_gsm_i[:, i], zorder=n_tid + i, color=c_fpi,
                  label=f"$V_{{{comps_[i]}i}}$")
        plot_line(axs11[2], v_gsm_p[:, i], zorder=n_tid + i, color=c_hpca,
                  label=f"$V_{{{comps_[i]}H^+}}$")

    axs11[2].legend(ncol=3, frameon=True, loc="upper right")
    axs11[2].set_ylim([-800, 2100])
    axs11[2].set_ylabel("$V_p$" + "\n" + "[km s$^{-1}$]")

    # Plot FPI-DIS, HPCA H+ and scaled HPCA He++ number densities
    plot_line(axs11[3], n_i, zorder=n_tid + 0, label="$n_i$")
    ...
    # Plot plasma beta
    plot_line(axs11[5], beta_, color="tab:blue")
    axs11[5].set_yscale("log")
    axs11[5].set_ylim([2e-2, 1.3e3])
    axs11[5].set_ylabel("$\\beta$" + "\n" + " ")
    axs11[5].axhspan(.02, .1, color="black", alpha=.2)
    axs11[5].axhspan(.1, .7, color="tab:red", alpha=.2)
    axs11[5].axhspan(.7, 1.3e3, color="tab:green", alpha=.2)
    axs11[5].text(.93, .85, "CPS", color="tab:green",
                  transform=axs11[5].transAxes)
    axs11[5].text(.93, .2, "PSBL", color="tab:red",
                  transform=axs11[5].transAxes)
    axs11[5].text(.93, .03, "Lobe", color="k", transform=axs11[5].transAxes)

    axs11[6], caxs6 = plot_spectr(axs11[6], dpf_omni_feeps_i[:, 2:],
                                  yscale="log", cscale="log",
                                  clim=[2e-1, 2e2], cmap="Spectral_r")
    axs11[6].set_ylabel("$K_i$" + "\n" + "[keV]")
    caxs6.set_ylabel("Diff. Flux" + "\n" + "[(cm$^{2}$ s sr keV)$^{-1}$]")

    axs11[7], caxs7 = plot_spectr(axs11[7], def_omni_fpi_i[:, 13:],
                                  yscale="log", cscale="log", clim=[1e3, 1e6],
                                  cmap="Spectral_r")
    axs11[7].set_ylabel("$K_i$" + "\n" + "[eV]")
    caxs7.set_ylabel("DEF" + "\n" + "[(cm$^{2}$ s sr)$^{-1}$]")

    axs11[8], caxs8 = plot_spectr(axs11[8], dpf_omni_feeps_e[:, 1:11],
                                  yscale="log", cscale="log", clim=[2e0, 2e3],
                                  cmap="Spectral_r")
    axs11[8].set_ylabel("$K_e$" + "\n" + "[keV]")
    caxs8.set_ylabel("Diff. Flux" + "\n" + "[(cm$^{2}$ s sr keV)$^{-1}$]")

    axs11[9], caxs9 = plot_spectr(axs11[9], def_omni_fpi_e[:, 9:],
                                  yscale="log", cscale="log",
                                  clim=[42.8e3, 42.8e6], cmap="Spectral_r")
    # axs11[4].axhline(def_omni_fpi_e.energy.data[7])
    axs11[9].set_ylabel("$K_e$" + "\n" + "[eV]")
    caxs9.set_ylabel("DEF" + "\n" + "[(cm$^{2}$ s sr)$^{-1}$]")
    ...
    make_labels(axs10, [.028, .92], pad=0)
    make_labels([axs01], [.028, .92], pad=2)
    make_labels(axs11, [.008, .84], pad=3)
